...
ROWS_WITH_NO_KWH_DATA = [5, 10, 23, 24, 27, 37, 41, 45, 46]
...
KWH_CHOICE = []
KWH_PROMPT_OPTIONS = ''
PROMPT_OPTIONS = KWH_PROMPT_OPTIONS
...
TRAINING_SET_ID = [35, 7, 15, 21, 13, 36, 0, 14, 11, 42, 20, 39, 1, 43, 9, 12, 38, 33, 8, 29, 44, 18, 17]
TEST_SET_ID = get_test_set_numbers(TRAINING_SET_ID + ROWS_WITH_NO_KWH_DATA)
...
def llm_evaluate_based_on_features(attributes, images, gt):
    ...
    try:
        ...
        if match:
            if EVALUATION_FEATURE == 'AGE' or EVALUATION_FEATURE == 'LIGHTING' or  EVALUATION_FEATURE == 'KWH':
        ...
        if EVALUATION_FEATURE == 'AGE' or EVALUATION_FEATURE == 'WINDOW' or EVALUATION_FEATURE == 'LIGHTING':
            gt = gt.lower().strip()
...
attributes = {0: ['Airtightness of windows and doors'], 1: [], 2: ['Energy-efficient glazing', 'Energy-efficient windows'], 3: ['Passive solar heating', 'Balcony with solar exposure'], 4: ['Wall-mounted thermostat'], 5: ['Ceiling spotlights'], 6: ['Stovetop'], 7: ['Hardwood floors']}
