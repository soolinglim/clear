...
EVALUATION_FEATURE = 'HEATING' #'HEATING' # 'KWH' # 'LIGHTING' #'AGE', 'WINDOW'
...
KWH_ESTIMATION_BLURB = 'Finally, give an estimate of the kwh. A highly efficient apartment might have a kwh/m2 value as low as 35 or better. An inefficient apartment might have a kwh/m2 value as high as 450 or worse. Always provide an estimate.'

AGE_FINAL_INSTRUCTIONS = WINDOW_FINAL_INSTRUCTIONS = LIGHTING_FINAL_INSTRUCTIONS = 'You can only use one of these, do not modify or invent your own options. Put the selected option in between ### and ###'
HEATING_FINAL_INSTRUCTIONS = AGE_FINAL_INSTRUCTIONS + '. Always select an option, even if you are unable to view or analyze the features in the images.'
...
def llm_evaluate_based_on_features(attributes, images, gt):
    ...
    try:
        ...
        if llm_repeated_call < MAX_LLM_RETRY: # allow for retries
            llm_repeated_call += 1
