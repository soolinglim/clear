...
EVALUATION_FEATURE = 'AGE' #'HEATING' # 'KWH' # 'LIGHTING' #'AGE', 'WINDOW'
...
AGE_FINAL_INSTRUCTIONS = WINDOW_FINAL_INSTRUCTIONS = LIGHTING_FINAL_INSTRUCTIONS = HEATING_FINAL_INSTRUCTIONS = 'You can only use one of these, do not modify or invent your own options. Put the selected option in between ### and ###'
...
AGE_CHOICE = ['early', 'mid-century', 'modern']
    ...
    try:
        ...
        if EVALUATION_FEATURE == 'AGE':
            ...
            if result == gt:
                ...
                if (gt == 'modern' and result == 'mid-century') or (gt == 'mid-century' and result == 'modern') or (gt == 'mid-century' and result == 'early') or (gt == 'early' and result == 'mid-century'):
                    gap = 1
                elif (gt == 'modern' and result == 'early') or (gt == 'early' and result == 'modern'):
                    gap = 2
                else:
                    raise Exception(f'No match found for age: {result}, {gt}')
        ...
        if llm_repeated_call < 2: # allow for retry twice
            llm_repeated_call += 1
...
def mutate_attributes(attributes):
    ...
    # mutation actions like swap and add may create duplicates, so remove duplicates
    attributes[random_attribute] = list(set(attributes[random_attribute])) 
